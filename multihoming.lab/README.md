...
_Выше я применил одинаковые конфиги на лифах leaf-02 и leaf-03. Разберу подробнее, что этот конфиг из себя представляет. На интерфейсе Ethernet 2 я всего лишь указываю принадлежность к Port-Channel 1, а на нем самом, помимо того, что я задаю trunk mode и allowed vlans я говорю лифу, что этим интерфейсом он смотрит на ES с идентификатором 0000:1111:1111:1111:0000. Также я указываю RT import <ДОПОЛНИТЬ>. Ну и в конце задаю LACP system-id, нужно это для того, чтобы host-03 видел в "show lacp peer" оба лифа как одно устройство. Проверим это_
