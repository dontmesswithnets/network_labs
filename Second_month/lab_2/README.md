...
* _Включаем функцию_ BGP _на NXOS, создаем процесс BGP и задаем AS (пример для dc1-sp-01)
```
feature bgp
router bgp 64501
```
...
```
  ...
  neighbor 169.254.0.5
    ...
    address-family ipv4 unicast
```
* Для соседей мы указываем их IP адрес, на который будет устанавливаться BGP соседство на 179 TCP порт, номер их автономной системы (в качестве лабораторной работы берем 16-битные номера AS), задаем описание и указываем какие политики address-family применить к соседу
...
* Здесь мы можем видеть, что маршрут до 10.1.0.1 dc1-lf-01 получает как от самого dc1-sp-01 c AS-Path 64501 i, так и от dc1-sp-o2 с AS-Path 64502 64504 64501 i. Это происходит потому, что dc1-sp-02 считакт такой маршрут до 10.1.0.1 лучшим, а соответственно, анонсирует его своим соседям. Аналогично с маршрутом до 10.1.0.2 от соседа dc1-sp-01 c AS-Path 64501 64504 64502 i, который dc1-sp-01 считает лучшим.
